...
const languageData = {
  ...
  lessons: { name:'Уроки', empty:'Уроков пока нет', invite:'Запланируйте первый урок', import_helper:'Вы можете импортировать CSV, чтобы добавить несколько уроков сразу.', import_format_hint:'Обязательные колонки: enrollment_id,instructor_id,vehicle_id,scheduled_time,duration_minutes,status (остальные игнорируются).', fields:{ id:'ИД', enrollment:'Запись', instructor:'Инструктор', vehicle:'Транспорт', scheduled_time:'Время', duration_minutes:'Длительность (мин)', status:'Статус', notes:'Заметки' } },
  'instructor-availabilities': { name: 'Доступность', empty: 'Нет доступности', invite: 'Добавьте доступность', fields: { id: 'ID', instructor_id: 'Инструктор', day: 'День', hours: 'Часы' } },

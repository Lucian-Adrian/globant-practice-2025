...
export default function VehicleListEmpty() {
  const t = useTranslate();
  const onImport = async (file) => {
    const form = new FormData();
    form.append('file', file);
    const resp = await rawFetch(`${API_PREFIX}/vehicles/import/`, { method: 'POST', body: form });
    if (!resp.ok) {
    ...
    if (json.errors && json.errors.length) {
      ...
      a.href = url; a.download = 'vehicle_import_errors.csv';
      document.body.appendChild(a); a.click(); a.remove();
      URL.revokeObjectURL(url);
      return { message: `Imported ${json.created} / errors: ${json.errors.length}` };
    }
    return { message: `Imported ${json.created} vehicles` };
  };

  return (
    <Box width="100%" mt={4} display="flex" justifyContent="center">
      <Stack spacing={2} maxWidth={600} textAlign="center">
        <Typography variant="h5">{t('resources.vehicles.empty', 'No vehicles yet')}</Typography>
        <Typography variant="body1" color="text.secondary">{t('resources.vehicles.import_helper', 'Import vehicles via CSV or create manually.')}</Typography>

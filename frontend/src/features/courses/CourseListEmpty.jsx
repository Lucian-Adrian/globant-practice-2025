...
export default function CourseListEmpty() {
  ...
  const onImport = async (file) => {
    ...
    const resp = await rawFetch(`${API_PREFIX}/courses/import/`, { method: 'POST', body: form });
    if (!resp.ok) {
    ...
    if (json.errors && json.errors.length) {
      ...
      a.href = url; a.download = 'course_import_errors.csv';
      document.body.appendChild(a); a.click(); a.remove();
      URL.revokeObjectURL(url);
      return { message: `Imported ${json.created} / errors: ${json.errors.length}` };
    }
    return { message: `Imported ${json.created} courses` };
  };

  return (
    <Box width="100%" mt={4} display="flex" justifyContent="center">
      <Stack spacing={2} maxWidth={600} textAlign="center">
        <Typography variant="h5">{t('resources.courses.empty', 'No courses yet')}</Typography>
        <Typography variant="body1" color="text.secondary">{t('resources.courses.import_helper', 'Import courses via CSV or create manually.')}</Typography>

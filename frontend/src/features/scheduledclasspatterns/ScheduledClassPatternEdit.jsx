...
const RecurrenceArrayInput = () => {
  ...
  return (
    <ArrayInput source="recurrences" defaultValue={initialRecurrences} inputProps={{ 'data-testid': 'recurrences' }}>
      <SimpleFormIterator>
    ...
    <>
      ...
      <Edit {...props} actions={<EditActions />}>
        <SimpleForm transform={(data) => {
          ...
          <TextInput source="name" label={t('resources.scheduledclasses.fields.name', 'Name')} validate={[required()]} inputProps={{ 'data-testid': 'name' }} />
          <ReferenceInput source="course_id" reference="classes" perPage={100}>
            <SelectInput label={t('resources.scheduledclasses.fields.course', 'Course')} optionText="name" optionValue="id" validate={[required()]} inputProps={{ 'data-testid': 'course_id' }} />
          </ReferenceInput>
          <ReferenceInput source="instructor_id" reference="instructors" perPage={100}>
            <SelectInput label={t('resources.scheduledclasses.fields.instructor', 'Instructor')} optionText={(r) => `${r.first_name} ${r.last_name}`} optionValue="id" validate={[required()]} inputProps={{ 'data-testid': 'instructor_id' }} />
          </ReferenceInput>
          <ReferenceInput source="resource_id" reference="resources" perPage={100}>
            <SelectInput label={t('resources.scheduledclasses.fields.resource', 'Resource')} optionText={(r) => r.name || r.license_plate} optionValue="id" validate={[required()]} inputProps={{ 'data-testid': 'resource_id' }} />
          </ReferenceInput>
          ...
          />
          <NumberInput source="num_lessons" label={t('resources.scheduledclasses.fields.num_lessons', 'Number of Lessons')} validate={[required()]} inputProps={{ 'data-testid': 'num_lessons' }} />
          <NumberInput source="duration_minutes" label={t('resources.scheduledclasses.fields.duration_minutes', 'Duration (min)')} defaultValue={60} inputProps={{ 'data-testid': 'duration_minutes' }} />
          <NumberInput source="max_students" label={t('resources.scheduledclasses.fields.max_students', 'Max Students')} inputProps={{ 'data-testid': 'max_students' }} />
          <SelectInput source="status" label={t('resources.scheduledclasses.fields.status', 'Status')} choices={statusChoices} inputProps={{ 'data-testid': 'status' }} />
          <ReferenceArrayInput source="student_ids" reference="students" perPage={100}>
            <SelectArrayInput label={t('resources.scheduledclasses.fields.students', 'Students')} optionText={(r) => `${r.first_name} ${r.last_name}`} inputProps={{ 'data-testid': 'student_ids' }} />
          </ReferenceArrayInput>
        </SimpleForm>
      </Edit>
    </>

...
const SignupForm = () => {
  const [form, setForm] = useState({ firstName:'', lastName:'', email:'', countryCode:'+373', localPhone:'', dob:'' });
  const [errors, setErrors] = useState({ firstName:null,lastName:null,email:null,countryCode:null,localPhone:null,dob:null });
  ...
  const formIsComplete = useMemo(() => {
    ...
    return Boolean(firstName.trim() && lastName.trim() && email.trim() && phoneLooksFilled && dob);
  }, [form]);

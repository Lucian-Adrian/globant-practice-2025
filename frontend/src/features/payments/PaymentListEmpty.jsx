...
export default function PaymentListEmpty() {
  const t = useTranslate();
  const onImport = async (file) => {
    const form = new FormData();
    form.append('file', file);
    const resp = await rawFetch(`${API_PREFIX}/payments/import/`, { method: 'POST', body: form });
    if (!resp.ok) {
    ...
    if (json.errors && json.errors.length) {
      ...
      a.href = url; a.download = 'payment_import_errors.csv';
      document.body.appendChild(a); a.click(); a.remove();
      URL.revokeObjectURL(url);
      return { message: `Imported ${json.created} / errors: ${json.errors.length}` };
    }
    return { message: `Imported ${json.created} payments` };
  };

  return (
    <Box width="100%" mt={4} display="flex" justifyContent="center">
      <Stack spacing={2} maxWidth={600} textAlign="center">
        <Typography variant="h5">{t('resources.payments.empty', 'No payments yet')}</Typography>
        <Typography variant="body1" color="text.secondary">{t('resources.payments.import_helper', 'Import payments via CSV or create manually.')}</Typography>
